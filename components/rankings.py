...
def render_ranking_cards(mp_data: List[Tuple[str, int, float]], parte_data: List[Tuple[str, int, float]]):
    """Renders side-by-side Top 3 ranking progress bars without emojis."""

    def build_ranking_items(data: List[Tuple[str, int, float]], color: str) -> str:
        if not data:
    ...
    mp_items = build_ranking_items(mp_data, COLORS['green_light'])
    parte_items = build_ranking_items(parte_data, COLORS['orange'])

    icon_mp = get_svg_icon("layers", size=16, color=COLORS["green_accent"])
    icon_parte = get_svg_icon("package", size=16, color=COLORS["orange"])
    ...
    ">
        ...
        ">
            <div style="
                ...
                margin-bottom: 16px;
                padding-bottom: 10px;
                border-bottom: 2px solid {COLORS['green_light']};
                display: flex;
                align-items: center;
                gap: 8px;
            ">
                {icon_mp}
                <span>Top 3 Linhas de Matéria-Prima</span>
            </div>
            {mp_items}
        </div>
        ...
        ">
            ...
            ">
                {icon_parte}
                <span>Top 3 Partes da Peça Solicitadas</span>
            </div>
            {parte_items}
        </div>
    </div>
